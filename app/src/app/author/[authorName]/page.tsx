import { getDoujinshiList } from '@/lib/data';
import type { Doujinshi } from '@/types';
import BookshelfView from '@/components/BookshelfView';

// searchParams の型定義
type SearchParams = {
  search?: string;
  genres?: string | string[];
};

// params の型定義
type PageParams = {
  authorName: string;
};

// props の型定義 (params と searchParams が Promise である可能性)
type PageProps = {
  params: Promise<PageParams>; // params を Promise に
  searchParams: Promise<SearchParams | undefined>; // searchParams も Promise に
};

export default async function AuthorBookshelfPage(props: PageProps) { // props を受け取る
  
  // ▼▼▼ props.params と props.searchParams を await ▼▼▼
  const resolvedParams = await props.params;
  const resolvedSearchParams = await props.searchParams;
  // ▲▲▲---------------------------------------------▲▲▲

  // URLデコードして元の作家名に戻す
  const authorName = decodeURIComponent(resolvedParams.authorName); 
  
  // await した値から検索パラメータを取得
  const search = resolvedSearchParams?.search || '';
  const rawGenres = resolvedSearchParams?.genres;
  const genres = Array.isArray(rawGenres)
    ? rawGenres
    : rawGenres ? [rawGenres] : [];
  
  let doujinshiList: Doujinshi[] = []; 
  try {
    // getDoujinshiList に author フィルタを渡す
    doujinshiList = await getDoujinshiList(search, genres, undefined, authorName); 
  } catch (error) {
    console.error(error);
  }

  return (
    <BookshelfView 
      items={doujinshiList} 
      currentSearch={search} 
      currentGenres={genres}
      pageTitle={`${authorName}`}
      filterContext={{ type: 'author', value: authorName }}
    />
  );
}